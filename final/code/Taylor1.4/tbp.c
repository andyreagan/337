...
int taylor_step_tbp(MY_FLOAT *ti,
                 MY_FLOAT *x,
                 int      dir,
                 int      step_ctl,
                 double   log10abserr,
 ...
 * 1: ok, and ti=endtime.  */
{
  MY_FLOAT **taylor_coefficients_tbp(MY_FLOAT, MY_FLOAT*, int);
  MY_FLOAT **taylor_coefficients_tbpA(MY_FLOAT, MY_FLOAT*, int, int);
  int compute_order_1_tbp(double, double, double, int*);
  int comp_order_other_tbp(double, double, double);
  double compute_stepsize_1_tbp(MY_FLOAT**, int, double, int);
  double compute_stepsize_2_tbp(MY_FLOAT**, int, double, int);
  double comp_stepsize_other_tbp(MY_FLOAT**, int, int, double, double, double);
...
*/
  switch(step_ctl)
    ...
    case 0: /* no step size control, fixed degree; both given by the user */
      ...
      break;
    case 1:
      nt=compute_order_1_tbp(xnorm,log10abserr,log10relerr,&flag_err);
      break;
    case 2:
      nt=compute_order_1_tbp(xnorm,log10abserr,log10relerr,&flag_err);
      break;
    case 3:
      nt=comp_order_other_tbp(xnorm,log10abserr,log10relerr);
      break;
    default:
...
*/
  if(step_ctl != 0) {
    s=taylor_coefficients_tbpA(*ti,x,nt,1);
  } else {
    s=taylor_coefficients_tbp(*ti,x,nt); 
 }
...
*/
  switch(step_ctl)
    ...
    case 0: /* no step size control (fixed step size, given by the user) */
      ...
      break;
    case 1:
      dh=compute_stepsize_1_tbp(s,nt,xnorm,flag_err);
      MakeMyFloatA(h, dh);
      break;
    case 2:
      dh=compute_stepsize_2_tbp(s,nt,xnorm,flag_err);
      MakeMyFloatA(h, dh);
      break;
    case 3:
      dh=comp_stepsize_other_tbp(s,_N_DIM_,nt,xnorm,log10abserr,log10relerr);
      MakeMyFloatA(h, dh);
      break;
    default:
      fprintf(stderr, "taylor_step: undefined step size control.\n");
...
int compute_order_1_tbp(double xnorm, double log10abserr, double log10relerr, int* flag_err)
/*
...
double compute_stepsize_1_tbp(MY_FLOAT **s, int nt, double xnorm, int flag_err)
/*
 ...
 */
{
  double double_log_MyFloat_tbp(MY_FLOAT x);
...
*/
  if (MyFloatA_LE_B(v1,of) && MyFloatA_GE_B(v1,uf))
    ...
    {
      lnv1=double_log_MyFloat_tbp(v1);
    }
  if (MyFloatA_LE_B(v2,of) && MyFloatA_GE_B(v2,uf))
    ...
    {
      lnv2=double_log_MyFloat_tbp(v2);
    }
/*
...
double compute_stepsize_2_tbp(MY_FLOAT **s, int nt, double xnorm, int flag_err)
/*
 * it looks for a step size for an expansion up to order nt. this
 * function requires that nt is the value computed by
...
int comp_order_other_tbp(double lnxnorm, double log10abserr, double log10relerr){
  puts("---");
...
double comp_stepsize_other_tbp(MY_FLOAT **s, int nd, int nt, double xnorm, double log10abserr, double log10relerr) {
...
#include <stdio.h>
#include <stdlib.h>
MY_FLOAT **taylor_coefficients_tbpA(MY_FLOAT t, MY_FLOAT *x, int order, int rflag)
{
   /* input: 
    ...
    static MY_FLOAT     _jz_cvars[1];
    static MY_FLOAT     *_jz_jet[12],  *_jz_save = NULL, *_jz_oneOverN=NULL,*_jz_theNs=NULL;
    ...
    if(_jz_maxOrderUsed < order )  { 
    	 ...
    	 if(_jz_ginitialized) {
    	    for(_jz_i=0; _jz_i<(_jz_oorder+1)*(12); _jz_i++) { ClearMyFloat(_jz_save[_jz_i]);} free(_jz_save);
    	 }
    	 _jz_save = (MY_FLOAT *)malloc((order+1)* 12 *sizeof(MY_FLOAT));
    	 for(_jz_i=0; _jz_i<(order+1)*(12); _jz_i++) { InitMyFloat(_jz_save[_jz_i]);}
    ...
    if(rflag == 0) {
	 ...
	 /* exponentiation: v_025=(v_010^i_016) */
		 ...
		 { int n=2, m, mn=0; 
		   switch(n) {
			  case 0: AssignMyFloat(_jz_jet[5][0], _jz_oneOverN[0]); break;
			  case 1: AssignMyFloat(_jz_jet[5][0], _jz_svar5); break;
			  case 2: AssignMyFloat(_jz_svar1, _jz_svar5); MultiplyMyFloatA(_jz_jet[5][0],_jz_svar1,_jz_svar5); break;
			  case 3: AssignMyFloat(_jz_svar1, _jz_svar5); MultiplyMyFloatA(_jz_svar2,_jz_svar1,_jz_svar5);
			          MultiplyMyFloatA(_jz_jet[5][0],_jz_svar1,_jz_svar2); break;
			  default:
			   AssignMyFloat(_jz_svar1, _jz_oneOverN[0]); AssignMyFloat(_jz_svar2, _jz_svar5);
			     while(mn==0) {
			        m=n; n /=2; if(n+n != m) {
			           ...
			           if(n==0){ mn=1;     AssignMyFloat(_jz_jet[5][0],_jz_svar1);}
			         }
	 ...
	 /* exponentiation: v_026=(v_011^i_016) */
		 ...
		 { int n=2, m, mn=0; 
		   switch(n) {
			  case 0: AssignMyFloat(_jz_jet[6][0], _jz_oneOverN[0]); break;
			  case 1: AssignMyFloat(_jz_jet[6][0], _jz_svar5); break;
			  case 2: AssignMyFloat(_jz_svar1, _jz_svar5); MultiplyMyFloatA(_jz_jet[6][0],_jz_svar1,_jz_svar5); break;
			  case 3: AssignMyFloat(_jz_svar1, _jz_svar5); MultiplyMyFloatA(_jz_svar2,_jz_svar1,_jz_svar5);
			          MultiplyMyFloatA(_jz_jet[6][0],_jz_svar1,_jz_svar2); break;
			  default:
			   AssignMyFloat(_jz_svar1, _jz_oneOverN[0]); AssignMyFloat(_jz_svar2, _jz_svar5);
			     while(mn==0) {
			        m=n; n /=2; if(n+n != m) {
			           AssignMyFloat(_jz_svar3, _jz_svar1); MultiplyMyFloatA(_jz_svar1, _jz_svar3, _jz_svar2);
			           if(n==0){ mn=1;     AssignMyFloat(_jz_jet[6][0],_jz_svar1);}
			         }
			        if(mn==0) {AssignMyFloat(_jz_svar3, _jz_svar2);AssignMyFloat(_jz_svar4, _jz_svar2);
			                   MultiplyMyFloatA(_jz_svar2, _jz_svar3,_jz_svar4);}
			       }
	 ...
	 /* state variable 2: */
	 AssignMyFloat(_jz_jet[2][1], _jz_jet[9][0]);
	 /* state variable 3: */
	 AssignMyFloat(_jz_jet[3][1], _jz_jet[11][0]);
	}
	 ...
	 for(_jz_k = _jz_lastOrder; _jz_k < order; _jz_k++) {
		 ...
		 { /* exponentiation */
			 ...
			 if(parity==0) {
			     ...
			     AddMyFloatA(_jz_jet[5][_jz_k], tmp2, tmp1);
			 } else { 
		 ...
		 { /* exponentiation */
			 ...
			 for(_jz_l=0; _jz_l<_jz_k; _jz_l++) {
			     ...
			     ppk -= pq  ;
			     MultiplyMyFloatA(tmp1, _jz_jet[8][_jz_l],_jz_jet[7][_jz_k-_jz_l]);
		 ...
		 { /* division */
			 ...
			 for(_jz_l=1; _jz_l<=_jz_k; _jz_l++) {
			     MultiplyMyFloatA(tmp1, _jz_jet[8][_jz_l],_jz_jet[9][_jz_k-_jz_l]);
			     AssignMyFloat(tmp2, tmp);
			     AddMyFloatA(tmp, tmp2, tmp1);
			 }
			     AssignMyFloat(tmp2, tmp);
		 ...
		 { /* division */
			 ...
			 AssignMyFloat(tmp, _jz_MyFloatZERO);
			 for(_jz_l=1; _jz_l<=_jz_k; _jz_l++) {
			     MultiplyMyFloatA(tmp1, _jz_jet[8][_jz_l],_jz_jet[11][_jz_k-_jz_l]);
			     AssignMyFloat(tmp2, tmp);
			     AddMyFloatA(tmp, tmp2, tmp1);
			 }
			     AssignMyFloat(tmp2, tmp);
			 SubstractMyFloatA(tmp, _jz_jet[10][_jz_k], tmp2);
			 DivideMyFloatA(_jz_jet[11][_jz_k], tmp, _jz_jet[8][0]);
		 }
		 ...
		 /* state variable 2: */
		 DivideMyFloatByInt(_jz_jet[2][_jz_m], _jz_jet[9][_jz_k], _jz_m);
		 /* state variable 3: */
		 DivideMyFloatByInt(_jz_jet[3][_jz_m], _jz_jet[11][_jz_k], _jz_m);
		 _jz_initialized=1;
	 }
    _jz_lastOrder = order;
    _jz_ginitialized=1;
...
MY_FLOAT **taylor_coefficients_tbp(MY_FLOAT t, MY_FLOAT *x, int order)
{
    return(taylor_coefficients_tbpA(t,x,order,0));
}
