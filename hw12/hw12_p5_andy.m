...
r= 0.1; %input('enter 0.5, 0.4, or 0.3 :   r = ') ;              %  K/h^2
tmax=0.5;
...
for kk=1:3          % this loop changes the step size in  x
	h=(1/20)/2^(kk-1);  %% looks complicated, but just 0.05,0.025,0.0125
	K=r*h^2;
    % fprintf('p = %g, r = %g, kappa = %g, h = %g\n',p,r,K,h)
    ...
    for n=1:N-1
        U(n+1,2:M-1)=r*U(n,3:M)+(1-2*r)*U(n,2:M-1)+r*U(n,1:M-2);
    end
